//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Pegasus.DAL
{
    using System;
    using System.Collections.Generic;
    
    public partial class UploadEnstaseis
    {
        public UploadEnstaseis()
        {
            this.UploadEnstaseisFiles = new HashSet<UploadEnstaseisFiles>();
        }
    
        public int UPLOAD_ID { get; set; }
        public Nullable<int> PROKIRIXI_ID { get; set; }
        public Nullable<int> AITISI_ID { get; set; }
        public string TEACHER_AFM { get; set; }
        public Nullable<int> SCHOOL_ID { get; set; }
        public Nullable<System.DateTime> UPLOAD_DATE { get; set; }
        public string UPLOAD_NAME { get; set; }
        public string UPLOAD_SUMMARY { get; set; }
    
        public virtual ICollection<UploadEnstaseisFiles> UploadEnstaseisFiles { get; set; }
    }
}
